...
export default function withSidebarComponent(Component){
    return function SidebarComponent(){
        const navigate = useNavigate()
        const {state} = useStore()
        const [active, setActive] = useState(false)
        useEffect(()=>{
            if(state.user == null || state.user.username === "")
                navigate('/login')
        },[])
            ...
            <>
                <aside className={"sidebar-nav-wrapper"+(active?" active":"")}>
                    <div className="navbar-logo">
                        <Link to="/">
                            <img src="/logo.png" alt="logo" style={{maxHeight:"150px", maxWidth:"150px"}} />
                        </Link>
                        {/* <a href="index.html">
                    ...
                    <nav className="sidebar-nav">
                        <ul>
                            ...
                            <li className="nav-item nav-item-has-children">
                                <a
                                    href="#0"
                                    className={(location.pathname!=="/"?"collapsed":"")}
                                    data-bs-toggle="collapse"
                                    data-bs-target="#menu_transaksi"
                                    aria-controls="menu_transaksi"
                                    // aria-expanded={"false"}
                                    aria-expanded={(location.pathname==="/"?"true":"false")}
                                    aria-label="Toggle navigation"
                                    >
                                    <span className="text">Transaksi</span>
                                </a>
                                <ul id="menu_transaksi" className={"collapse dropdown-nav"+(location.pathname==="/"?" show":"")}>
                                    <li>
                                        <Link to="/" className={(location.pathname==="/"?"active":"")}>Checklist Pencairan</Link>
                                    </li>
                                </ul>
                            </li>
                            <li className={"nav-item "+(location.pathname==="/report"?"active":"")}>
                                <Link to="/report">
                                <span className="text">Laporan</span>
                                </Link>
                ...
                <main className={"main-wrapper"+ (active?" active":"")}>
                    <Header click={sidebarClick} active={active}/>
